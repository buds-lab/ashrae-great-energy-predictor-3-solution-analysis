...
if __name__ == "__main__":
    ...
    with timer("Predicting"):
        ...
        for s in range(16):
            ...
            if args.normalize_target:
                sub_model_path = f"{MODEL_PATH}/cb-split_site/target_normalization/site_{s}"
            else:
                sub_model_path = f"{MODEL_PATH}/cb-split_site/no_normalization/site_{s}"
    ...
    # save data
    if args.normalize_target:
        np.save(f"{OUTPUT_PATH}/cb-split_site-target_normalization", test_preds)
    else:
        np.save(f"{OUTPUT_PATH}/cb-split_site-no_normalization", test_preds)